...
class CodeGenerationChatAgent:
    ...
    def initialize(self, api_key=None):
        import os
        # Priority: 1. Argument, 2. Secrets, 3. Env Var
        if api_key:
            self.api_key = api_key
        elif "GEMINI_API_KEY" in st.secrets:
            self.api_key = st.secrets["GEMINI_API_KEY"]
            print(f"🔑 Loaded API Key from Secrets (Project: Soldado)")
        else:
    ...
    def ask(self, question, history=None):
        ...
        - "Perforadora" / "Perforacion" -> Use DataFrame 'perfos'. 
             * IMPORTANT: To make a table, PIVOT on 'MetricCategory', NOT 'Metric'.
             * IMPORTANT: To make a table, PIVOT on 'MetricCategory', NOT 'Metric'.
             * CRITICAL: The 'perfos' DataFrame ALREADY contains ONLY drilling equipment. 
             * DO NOT filter the 'Item' column for strings like "Perforadora" or "Drill". YOU WILL GET EMPTY RESULTS.
             * JUST group by 'Item' (values are like 'PV-5', 'D65', etc).
             * COMPLEX AGGREGATION RULE: `pivot_table` cannot handle mixed sum/mean for column values.
             * SOLUTION:
               1. Filter & Pivot SUM metrics: 
                  `df_sum = df[df['MetricCategory'].str.contains('Horas|Ton|Mts')].pivot_table(index='Item', columns='MetricCategory', values='Value', aggfunc='sum')`
               2. Filter & Pivot MEAN metrics: 
                  `df_mean = df[df['MetricCategory'].str.contains('Uso|Disp|%')].pivot_table(index='Item', columns='MetricCategory', values='Value', aggfunc='mean')`
               3. JOIN them: `result = df_sum.join(df_mean, how='outer')`
             * ALWAYS `reset_index()` after joining.
             * CRITICAL: ALWAYS use `columns='MetricCategory'`. NEVER use `columns='Metric'`.
             * CRITICAL: DO NOT DROP COLUMNS. Return the FULL table with all pivot columns.
             * CRITICAL: DO NOT RENAME COLUMNS unless explicitly asked.
             * CRITICAL: If user asks for "Uso", INCLUDE ALL columns containing "Uso" (e.g., 'Uso de la disponibilidad(%)', 'Uso Con Neblina', etc.). DO NOT PICK JUST ONE.
        - "Servicios" / "Apoyo" -> Use DataFrame 'servicios'. Same rules as Perfos (Already filtered).
        - "Horas" (Perfos/Servicios) -> Filter `df['MetricCategory'].str.contains('Horas|Operativas', case=False)`
        - "Uso" / "Disponibilidad" -> Filter `df['MetricCategory'].str.contains('Uso|Disponibilidad', case=False)`
        """
