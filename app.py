...
@st.cache_data
def load_data_v6(file_path):
    # file_path is passed as argument now
    
    import ai_loader # Required for AI Context
    try:
        # 1. Planta
        ...
        def get_col_map(df, sheet_name):
            ...
            for c in range(df.shape[1]):
                y_val_raw = str(df.iloc[y_row, c]).replace('.0','').strip()
        ...
        try:
            ...
            for _, row in df_costos.iterrows():
                # PARANOID TYPE CASTING: int() and str()
                try:
                    k_y = int(row['Year'])
                    k_p = str(row['Period_Clean'])
                    key = (k_y, k_p)
            ...
            # EMERGENCY HARDCODE 2028/2029 (As per User Screenshot)
            # 2028
            for q in ['Q1', 'Q2', 'Q3', 'Q4']:
                cost_lookup[(2028, q)] = {'mina': 2.92, 'planta': 13.03}
            # 2029 (Q1 only visible in screenshot, assuming rest similar or Q1 only)
            cost_lookup[(2029, 'Q1')] = {'mina': 2.35, 'planta': 11.78}
            # Fill rest of 2029 keys just in case
            for q in ['Q2', 'Q3', 'Q4']:
                 if (2029, q) not in cost_lookup:
                     cost_lookup[(2029, q)] = {'mina': 2.35, 'planta': 11.78}
        ...
        for b in blocks:
            ...
            # COST LOOKUP
            # COST LOOKUP (PARANOID ACCESS)
            # c_mina = [cost_lookup.get((int(y), str(l)), {}).get('mina', 0.0) for l in lbls]
            # c_planta = [cost_lookup.get((int(y), str(l)), {}).get('planta', 0.0) for l in lbls]
            
            c_mina = []
            c_planta = []
            for l in lbls:
                val_m = cost_lookup.get((int(y), str(l)), {}).get('mina', 0.0)
                val_p = cost_lookup.get((int(y), str(l)), {}).get('planta', 0.0)
                # DEBUG 2028 LOOP
                # if int(y) == 2028:
                #    if val_m == 0:
                #        st.warning(f"⚠️ 2028 Cost Missing for {l}! Key looked for: ({int(y)}, '{str(l)}')")
                
                # --- BRUTE FORCE OVERRIDE (SOLUCION FINAL) ---
                if int(y) == 2028:
                    val_m = 2.92
                    val_p = 13.03
                elif int(y) == 2029:
            ...
            df_b = pd.DataFrame({
                ...
                'Trat_Planta': extract_series(df_planta, r_trat, cols_p, div_by=1.0), # WAS 1000.0 - REVERT TO TONS (T)
        ...
        # No need for separate context Unless we want robust robust access.
        # But for 'dashboard' context, it's implicit.
        
        # FINAL DEBUG: PRINT DF SAMPLE
        ...
        return {
            ...
            'perfos': df_perfos,
            'servicios': df_serv,
            'ai_raw': ai_loader.get_ai_context(file_path) # Added AI Context to V6
        }
...
# We unpack it safely.
try:
    # _global_data = load_data_v5()
    # df = _global_data.get('planta', pd.DataFrame()) if _global_data else pd.DataFrame()
    df = pd.DataFrame() # Placeholder to prevent NameError, strictly overwritten later
...
def render_dashboard(df, df_pala_fase_view, df_fleet=None, key_id="main"):
    ...
    # --- SIMULADOR DE NEGOCIO (SIDEBAR) ---
    with st.sidebar.expander("🎰 SIMULADOR DE NEGOCIO", expanded=False):
        st.info("Ajusta las variables para ver el impacto financiero.")
        sim_price = st.slider("💰 Precio Cobre ($/lb)", 2.0, 6.0, 4.4, 0.1, key=f"sim_p_{key_id}")
        sim_rec_delta = st.slider("⚗️ Delta Recuperación (%)", -5.0, 5.0, 0.0, 0.1, key=f"sim_r_{key_id}")
        sim_cost_fac = st.slider("📉 Factor Costos (%)", 80, 120, 100, 5, key=f"sim_c_{key_id}") / 100.0
        
        st.markdown("---")
        st.caption("Variables Ley de Corte")
        # Variables Ley de Corte
        sim_caa = st.slider("💧 Costo Aguas Abajo ($/lb)", 0.5, 1.5, 0.655, 0.005, key=f"sim_caa_{key_id}")
        sim_ctdif = st.slider("🚛 Diferencial Transporte ($/hr)", 150.0, 300.0, 237.0, 1.0, key=f"sim_ctdif_{key_id}")
        sim_cr = st.slider("🚜 Costo Remanejo ($/t)", 1.0, 2.0, 1.21, 0.01, key=f"sim_cr_{key_id}")
    
    # --- CÁLCULO FINANCIERO (BASE vs SIM) ---
    # Base Data
    base_ton_cu = safe_sum('Cobre_Fino')
    base_mov_mina = safe_sum('Mov_Total') * 1000 # Convert kTon to Ton
    base_trat_planta = safe_sum('Trat_Planta') # In Ton already? No, Row 14 is Ton. Check safe_sum usage.
    # Wait, safe_sum('Trat_Planta') in kpi calculation was divided by 1000 to get kTon. So it is in Ton.
    base_trat_planta_ton = safe_sum('Trat_Planta')

    # Financials Base (Est)
    # Revenue = TonCu * 2204.62 * Price(4.0 default)
    base_revenue = base_ton_cu * 2204.62 * 4.0 
    
    # Costs Base (Est)
    # Cost = (MovMina * CostMina) + (TratPlanta * CostPlanta)
    # Use dataframe columns for accurate weighted sum if available
    if 'Costo_Mina' in df.columns and 'Mov_Total' in df.columns:
        cost_mina_total = (df['Mov_Total'] * 1000 * df['Costo_Mina']).sum()
    else:
        cost_mina_total = base_mov_mina * 2.2 # Fallback $2.2/t
        
    if 'Costo_Planta' in df.columns and 'Trat_Planta' in df.columns:
        cost_planta_total = (df['Trat_Planta'] * df['Costo_Planta']).sum()
    else:
        cost_planta_total = base_trat_planta_ton * 12.0 # Fallback $12/t
    
    base_remanejo_ton = safe_sum('Remanejo') * 1000.0
    base_gastos_remanejo = base_remanejo_ton * 1.21 # Default CR
    
    # Base Core (Mina + Planta)
    base_cost_core = cost_mina_total + cost_planta_total 
    # Base Total (Now assumes Remanejo is inside Cost Mina)
    # Add Downstream Cost (User Formula: CostAA * 2.4 * Fino * 2204.62)
    # For Base, we use the default CostAA (0.655) and Base Fino
    base_downstream_cost = 0.655 * 2.4 * base_ton_cu * 2204.62
    base_cost_total = base_cost_core + base_downstream_cost
    ...
    # Recup Sim impact
    if avg_recup > 0:
        pct_change_rec = sim_rec_delta / avg_recup
        sim_ton_cu = base_ton_cu * ((avg_recup + sim_rec_delta) / avg_recup)
    else:
        sim_ton_cu = base_ton_cu

    sim_revenue = sim_ton_cu * 2204.62 * sim_price
    
    # Calculate Remanejo Cost (Just for display/KPIs if needed, but not adding to Total)
    # Remanejo kTon -> Ton
    sim_remanejo_ton = safe_sum('Remanejo') * 1000.0
    sim_gastos_remanejo = sim_remanejo_ton * sim_cr
    
    # Update Total Cost: Scale CORE only. Assumes users want to scale the 'Input Cost' which includes everything.
    sim_cost_core_scaled = base_cost_core * sim_cost_fac 
    
    # Add Downstream Cost for Simulation
    sim_downstream_cost = sim_caa * 2.4 * sim_ton_cu * 2204.62
    
    sim_cost_total_final = sim_cost_core_scaled + sim_downstream_cost
    
    sim_margin = sim_revenue - sim_cost_total_final
    
    
    # --- CUT-OFF GRADE CALCULATION (NEW FORMULA) ---
    # Formula: (Trat * CP + Trat * CTdif/231 + Rem * CR) / (2204.6226 * Price * Rec * Trat)
    # Result is a decimal (e.g. 0.004 for 0.4%)
    
    if base_trat_planta_ton > 0:
        sim_rec_final = (avg_recup + sim_rec_delta) / 100.0 # Decimal 0.XX
        
        # Components
        # We need Weighted Avg Cost Planta ($/t) for the scenario?
        # Or just use the total Cost Planta from DataFrame? 
        # Using aggregates:
        # Term 1: Trat * CostPlanta  -> This is essentially cost_planta_total (if available).
        # Let's derive 'avg_cost_planta_val' from Data or fallback.
        if 'Costo_Planta' in df.columns:
             term_cp = (df['Trat_Planta'] * df['Costo_Planta']).sum() # Total Cost Planta $
        else:
             term_cp = base_trat_planta_ton * 12.0 # Fallback
             
        # Scale Cost Planta by sim_cost_fac? User said "Trat Planta * Costo planta". 
        # Assuming we apply the global cost factor to this component too.
        term_cp_sim = term_cp * sim_cost_fac
        
        numerator = (term_cp_sim) + \
                    (base_trat_planta_ton * sim_ctdif / 231.0) + \
                    (sim_gastos_remanejo) 
                    
        denominator = 2204.6226 * (sim_price - sim_caa) * sim_rec_final * base_trat_planta_ton
        
        if denominator > 0:
            sim_cog = numerator / denominator
        else:
            sim_cog = 0.0
    else:
        sim_cog = 0.0
    ...
    delta_cog = sim_cog - (avg_ley / 100.0)
    # If COG > AvgLey, we are processing uneconomic material (BAD)
    ...
    # --- RESULTADOS SIMULACIÓN FINANCIERA ---
    st.markdown("### 🎰 Resultados Simulación Financiera")
    
    # Calculate Deltas using Final values
    delta_rev = sim_revenue - base_revenue
    delta_marg = sim_margin - base_margin
    delta_cost = sim_cost_total_final - base_cost_total
    delta_remanejo_cost = sim_gastos_remanejo - base_gastos_remanejo
    
    # Calculate Base COG for Comparison (MATCHING SIM FORMULA EXACTLY)
    # Base Defaults: Price=4.0, CAA=0.655, CTdif=237.0, CR=1.21
    if base_trat_planta_ton > 0:
        base_rec = avg_recup / 100.0
        
        # Base Component Costs
        if 'Costo_Planta' in df.columns:
             term_cp_base = (df['Trat_Planta'] * df['Costo_Planta']).sum()
        else:
             term_cp_base = base_trat_planta_ton * 12.0
             
        # Remanejo at default CR=1.21
        term_rem_base = base_remanejo_ton * 1.21
        
        # Transport Diff at default 237/231
        # term_trans_base = Trat * 237 / 231
        term_trans_base = base_trat_planta_ton * 237.0 / 231.0
        
        base_num = term_cp_base + term_trans_base + term_rem_base
        base_den = 2204.6226 * (4.0 - 0.655) * base_rec * base_trat_planta_ton
        
        base_cog = base_num / base_den if base_den > 0 else 0.0
    else:
        base_cog = 0.0

    delta_cog_sim = sim_cog - base_cog

    col_fin1, col_fin2, col_fin3, col_fin4, col_fin5 = st.columns(5)
    
    with col_fin1:
        st.metric("💰 Ingresos Estimados", fmt_m(sim_revenue), f"{fmt_m(delta_rev)} vs Base", delta_color="normal")
    with col_fin2:
        st.metric("📉 Gastos Totales", fmt_m(sim_cost_total_final), f"{fmt_m(delta_cost)} vs Base", delta_color="inverse")
    with col_fin3:
         # Color logic: Green if Margin improved (delta > 0), Red if worsened
         st.metric("💵 OFCF", fmt_m(sim_margin), f"{fmt_m(delta_marg)} vs Base", delta_color="normal")
    with col_fin4:
        # COG Metric (Convert Decimal to %) Comparing vs Base Scenario
        val_cog_pct = sim_cog * 100.0
        delta_cog_pct = delta_cog_sim * 100.0
        st.metric("✂️ Ley de Corte", f"{val_cog_pct:.2f}%", f"{delta_cog_pct:+.2f}% vs Base", delta_color="inverse")
    with col_fin5:
        st.metric("🚜 Gastos Remanejo Total", fmt_m(sim_gastos_remanejo), f"{fmt_m(delta_remanejo_cost)} vs Base", delta_color="inverse")
    ...
    with tab1:
        ...
        with g1:
            ...
            # --- DEFINIMOS LOS EJES Y FORMATOS COMUNES ---
            x_axis = alt.X('Periodo', 
                sort=None,
                type='ordinal',
                axis=alt.Axis(
                    labelAngle=-90,
            ...
            # --- CAPA 2: ETIQUETAS (COBRE FINO) ---
            # Separated Chart to avoid inheritance issues
            text_bars = alt.Chart(df).mark_text(
                align='center',
                baseline='middle',
                dy=-10, # Shift up slightly
                color='white',
                angle=-90 # Vertically
            ).encode(
                x=x_axis,
            ...
            line = alt.Chart(df).mark_line(color='#1f77b4', strokeWidth=3).encode(
                ...
                tooltip=['Periodo', 'Ley_CuT']
            )
            
            # --- CAPA 4: ETIQUETAS (LEY CUT) ---
        ...
        with g2:
            ...
            if valid_melt:
                ...
                # --- CAPA 2: ETIQUETAS INTERNAS ---
                text_mov = alt.Chart(df_melt).mark_text(
                    dy=0,
                    color='white',
                    fontSize=9
                ).encode(
                    ...
                    opacity=alt.condition(alt.datum.Kton > 50, alt.value(1), alt.value(0))
                )
        ...
        if 'Trat_Planta' in df_treat.columns:
            ...
            # --- CAPA 2: ETIQUETAS ---
            text_treat = alt.Chart(df_treat).mark_text(
                align='center', 
                baseline='bottom',
                dy=-5,
                color='white',
...
def render_fleet_dashboard(df_fleet):
    ...
    # --- ANIMACIÓN FLOTA (A Petición Usuario: "Palas y Fases") ---
    st.markdown("### 🚜 Evolución Dinámica (Time-Lapse)")
    st.markdown("#### ¿Dónde están las Palas?")
    
    # 1. Year Selection
    available_years = sorted(df_fleet['Year'].unique().tolist())
    if not available_years:
         available_years = [2026] # Fallback
         
    # Container for controls
    c_anim1, c_anim2 = st.columns([1, 3])
    with c_anim1:
        selected_year_anim = st.selectbox("📅 Año", options=available_years, index=0)
    
    # 2. Filter Data
    df_anim_fleet = df_fleet[df_fleet['Year'] == selected_year_anim].copy()
    
    valid_loaders = ['Pala 06', 'Pala 05', 'Pala 04', 'Pala 03', 'Cargador Frontal']
    
    if not df_anim_fleet.empty and 'Fase' in df_anim_fleet.columns:
        # 3. Robust Data Prep & Labels
        # Logic: If Year >= 2028 (or Periodo starts with Q), use 'Periodo'. Else use Month Map.
        
        # Month Map for 2026-2027
        month_map = {1: 'Enero', 2: 'Febrero', 3: 'Marzo', 4: 'Abril', 5: 'Mayo', 6: 'Junio', 
                     7: 'Julio', 8: 'Agosto', 9: 'Septiembre', 10: 'Octubre', 11: 'Noviembre', 12: 'Diciembre'}
        
        if selected_year_anim >= 2028:
             # USE QUARTER/PERIOD LABELS DIRECTLY
             df_anim_fleet['MonthLabel'] = df_anim_fleet['Periodo']
             # Sort key: Assuming format "Q1 2029" is sortable. If not, we might need custom sort.
             # "Q1 2029" sorts correctly alphabetically.
             custom_order = sorted(df_anim_fleet['Periodo'].unique().tolist())
        else:
             # USE MONTH NAMES
             if df_anim_fleet['Month'].dtype != 'object':
                  df_anim_fleet['MonthLabel'] = df_anim_fleet['Month'].map(month_map)
             else:
                  df_anim_fleet['MonthLabel'] = df_anim_fleet['Month']
             custom_order = list(month_map.values())
             
        # Filter only valid loaders
        df_anim_filt = df_anim_fleet[df_anim_fleet['Equipo'].isin(valid_loaders)].copy()
        
        if not df_anim_filt.empty:
            # Drop invalid months
            df_anim_filt = df_anim_filt.dropna(subset=['MonthLabel'])
            
            # Categorical Sort to Ensure Timeline Order
            # Only use categories present in data to avoid empty frame errors
            present_labels = df_anim_filt['MonthLabel'].unique().tolist()
            final_order = [x for x in custom_order if x in present_labels]
            
            if not final_order: # Fallback if intersection is empty
                final_order = sorted(present_labels)
            
            df_anim_filt['MonthLabel'] = pd.Categorical(df_anim_filt['MonthLabel'], categories=final_order, ordered=True)
            df_anim_filt = df_anim_filt.sort_values('MonthLabel')
            
            # Calc Dynamic Range
            max_ton = df_anim_filt['Ton'].max()
            if pd.isna(max_ton) or max_ton <= 0: max_ton = 100
            y_upper = max_ton * 1.15
            ...
            fig_anim_f = px.bar(
                ...
                range_y=[0, y_upper], # Safe Dynamic Range
                title=f"Despliegue ({selected_year_anim})",
                color_discrete_map={
                    'Pala 06': '#ff9f43', 
                    'Pala 05': '#ee5a24', 
                    'Pala 04': '#0abde3', 
                    'Pala 03': '#5f27cd',
                    'Cargador Frontal': '#10ac84' # Added Color for CF
                }
            )
            
            # Layout fixes & REMOVE PLAY BUTTON (User Request)
            fig_anim_f.update_layout(
                 margin=dict(l=20, r=20, t=40, b=20),
                 height=500,
                 updatemenus=[dict(type="buttons", buttons=[])] # Hide Play Button essentially
            ) 
            # REMOVED: fig_anim_f.layout.updatemenus[0]... call that caused Index Error
            
            st.plotly_chart(fig_anim_f, use_container_width=True)
            
        else:
            st.info(f"Sin actividad de carguío (Palas) en {selected_year_anim}.")
    else:
        st.warning(f"No hay datos para {selected_year_anim}.")
...
import gc
gc.collect() # Force cleanup before heavy load

# --- LOAD DATA (V6 CACHED) ---
data_bundle = None
try:
    with st.spinner("Cargando modelo de datos..."):
        data_bundle = load_data_v6(FILE_PATH)
except Exception as e:
    st.error(f"❌ Error Crítico al Iniciar: {e}")
    st.info("Reintentando en modo seguro...")
    try:
        # Fallback to empty structure to allow UI to render at least
        data_bundle = {'planta': pd.DataFrame(), 'fleet': pd.DataFrame(), 'perfos': pd.DataFrame(), 'servicios': pd.DataFrame(), 'ai_raw': {}}
    except:
        st.stop()
...
if data_bundle:
    ...
    with t1: 
        if st.button("📈 2026-2029", use_container_width=True, type="primary" if st.session_state['nav_period']=="📈 2026-2029" else "secondary"): 
            st.session_state['nav_period'] = "📈 2026-2029"
            st.rerun()
    with t2: 
        if st.button("📅 2026", use_container_width=True, type="primary" if st.session_state['nav_period']=="2026" else "secondary"): 
