...
from chat_agent import MiningChatAgent
    ...
    if current_view == "📊 Panel de Control":
        ...
        if 'chat_agent_inst' not in st.session_state:
            try:
                ...
                agent = MiningChatAgent(data_context)
        ...
        if not st.session_state.messages:
            ...
            suggestions = [
                "Muestra el movimiento de mina por fases para el año 2026",
                "¿Cuál es el tratamiento de planta acumulado para el periodo 2026-2029?",
                "Compara la ley media de cobre entre 2026 y 2027",
                "Genera una tabla detallada del Cobre Fino por trimestre"
            ]
            
            for s in suggestions:
                if st.button(f"✨ {s}", key=f"sug_{s}", use_container_width=False, type="secondary"):
                    st.session_state.messages.append({"role": "user", "content": s, "type": "text"})
                    st.rerun()
        else:
